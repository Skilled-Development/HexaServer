...
func ReadStatusStatePacket(server_config config.ServerConfig, p player.Player, length int32, packet_id int32, pack *packets.PacketReader) {
	switch packet_id {
	case 0x00:
...
func handle_ping_request_packet(p player.Player, pack *packets.PacketReader) {
	...
	clientbound.NewPongResponsePacket_1_21(timestamp).GetPacket().Send(p)
}

func handle_status_request_packet(p player.Player, server_config config.ServerConfig) {
	motd := server_config.GetMOTD()
	clientbound.NewStatusResponsePacket_1_21(&motd).GetPacket().Send(p)
}
